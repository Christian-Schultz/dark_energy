...
void fill_write_buffer(enum iofields blocknr, int *startindex, int pc, int type)
{
  ...
  switch (blocknr)
	...
	if(P[pindex].Type == type)
	  {
#if defined ISOTHERM_EQS || defined NEUTRINO_FLUID
...
/* Insert statement to take care of HSML and RHO for NUSPH (exclude it in snapshot if NEUTRINO_FLUID is defined) ? */
